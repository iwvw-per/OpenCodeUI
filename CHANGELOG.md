...
- fix(header): 打开项目目录在远程实例位于本机时改用系统文件管理器 (0072d777)

## [v0.6.102] - 2026-09-24

- feat(project): 目录弹窗的系统文件夹按钮不再限制焦点服务器，云端/远程场景也能从资源管理器选目录 (870a4423)

## [v0.6.101] - 2026-09-24

- perf(i18n,bundle): 翻译资源按语言懒加载 + CodeMirror/Radix 独立分包 + 深导入瘦身首屏；修复首帧语言被兜底成 en 并覆盖偏好 (ab966bd0)

## [v0.6.100] - 2026-09-24

- fix(multi-server): 底部主机条切换后焦点服务器跟随活动服务器，修复新建项目仍显示旧主机目录 (d5776234)

## [v0.6.99] - 2026-09-24

- feat(project): 目录弹窗加大 + 加系统文件夹选择器 + 活动目录按服务器分区 (6520ba18)

## [v0.6.98] - 2026-09-24

- fix(multi-server): 远程目录选择/打开跟随服务器 + 项目展开状态按服务器分区 + 修复切换后全部展开 (2fa2640a)

## [v0.6.97] - 2026-09-24

- fix(pane): 远程主机添加项目改用内置目录选择器，不再弹出本机文件夹 (76ff8646)

## [v0.6.96] - 2026-09-23

- fix(pane): 拖拽柄松手后显式写回 grid 模板，修复双击后分屏塌成上下布局 (fff8b92e)
...
- fix(sync): 界面开合状态不再跨端同步，消除侧栏/面板互相折叠 (5e226ede)

## [v0.6.92] - 2026-09-22

- fix(sync): 拉取不再覆盖尚未上传的本地改动，修复打开侧栏被同步折叠回去 (3a0f0994)

## [v0.6.91] - 2026-09-22

- feat(sync): 同步成功图标改为全亮小方格，与同步中同款几何 (80e540fa)

## [v0.6.90] - 2026-09-22

- fix(mobile): 移动端侧栏开合不再落盘同步，修复关闭后被同步弹回 (fb01b8e0)
- feat(chat): 提问超时自动选择 + 变更 chip 跳转面板 + 修复项目排序与展开状态同步 (3758b2f4)

## [v0.6.89] - 2026-09-22

- fix(sync): 拉取后让 layoutStore 重读，修复排序等偏好不同步 (ce3cab01)

## [v0.6.88] - 2026-09-22

- fix(sync): 消除写回时间窗，修复新建项目被同步冲掉 (2ea87ff0)

## [v0.6.87] - 2026-09-22

- fix(sync): 修复新建项目被本地状态覆盖丢失 + 同步图标移到主机行右侧 (8e5e79fd)

## [v0.6.86] - 2026-09-22

- feat(sync): 本地改动即时上传 + 侧栏底部同步状态行 (f0e93c72)

## [v0.6.85] - 2026-09-22

- fix(sync): 改为三方合并（基线快照），修复删除被复活与新增被丢弃 (e63a72cc)

## [v0.6.84] - 2026-09-22

- fix(sync): 拉取后通知 UI 重读 + 撤销跨实例合并（修复同步看起来没生效） (6839da73)

## [v0.6.83] - 2026-09-21

- fix(sync): 修复跨实例并集复活已删项目 + 清理死代码 (d3294d53)
- feat(sync): 项目列表只认已保存项目 + SSE 实时同步 (bbb5681b)
- fix(sync): 已保存项目目录跨实例聚合 + 清理陈旧实例服务器 (fa246b99)

## [v0.6.82] - 2026-09-21

- feat(sessions): 已归档对话支持批量选择与删除 (1745a265)
- fix(header): 修复桌面端标题栏拖拽区与标题宽度 (409a32ee)
- fix(sync): 服务端体积限制前置拦截 + 同步入口统一互斥 + 清理诊断代码 (77bf19f6)
- fix(sync): 多端偏好同步改白名单 + 双向 + 时间戳仲裁 + 合并语义 (1c521e67)

## [v0.6.81] - 2026-09-21

- style(ui): 全站统一交互态文字样式 — 按钮/组件 hover、active、选中、展开时文字不加粗、不缩放、不改色；行内可点击文字改为「圆角边框 + 背景」提示，字体效果不变（图标除外）(HEAD)

## [v0.6.80] - 2026-09-19

- perf(session): 历史页裁剪重负载字段 + 开启响应压缩 (9d637c39)

## [v0.6.79] - 2026-09-19

- perf(session): 历史消息改游标分页，上滑加载从 O(N²) 降为 O(N) (700854aa)

## [v0.6.78] - 2026-09-18

- fix(message): 过程折叠壳补回负 margin 补偿，修复步骤行 hover 圆角与内边距被裁 (ad4661af)
